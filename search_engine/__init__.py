...
def query(req):
    Features, Weights, Contraints = req
    similarity = {}

    min_array = np.array([min_display, min_camera, min_popular, min_price])
    max_min_array = np.array([max_display, max_camera, max_popular, max_price]) - np.array(
        [min_display, min_camera, min_popular, min_price])
    for key in database:
