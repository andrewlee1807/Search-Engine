...
class Display:
    ...
    def from_dict(obj: Any) -> 'Display':
        ...
        protection = from_str(obj.get("Protection"))
        if protection is not None:
            protection = 1
        else:
            protection = 0
        display = from_str(obj.get("Display"))
        return Display(type, size, resolution, protection, display)
...
class Launch:
    ...
    def from_dict(obj: Any) -> 'Launch':
        ...
        return Launch(announced, status)
    ...
    def from_dict(obj: Any) -> 'Camera':
        ...
        if value is not None and value != '' and len(value.split(" ")) > 1:
            ...
            try:
                ...
                pass
        return Camera(features, video, single, quad)
...
class Device:
    ...
    def from_str(obj: Any) -> 'Device':
        ...
        return Device(brand, product_name)
...
# Opening JSON file
with open(r"D:\OneDrive - Chonnam National University\Master\Topics on Web Mining\HW\Teamwork\code\phone.json",
...
list_device[123].device.brand
list_device[123].device.product_name

# popularity
t = [t.popularity for t in list_device]
love = [i.love for i in t]
# love
love_int = []
for i in love:
    if i != '':
        love_int.append(int(i))
    else:
        love_int.append(0)

np.unique(love, return_counts=True)

np.average(love_int)  # 133.61984732824428

click = [i.click for i in t]
# love
click_int = []
for i in click:
    if i != '':
        click_int.append(int(i))
    else:
        click_int.append(0)

np.average(click_int)  # 133.61984732824428

click_int = []
for i in click:
    if i != '':
        try:
            click_int.append(float(i.replace(',', '')))
        except:
            click_int.append(0)
    else:
        click_int.append(0)
np.average(click_int)  # 1530783.3888040714
